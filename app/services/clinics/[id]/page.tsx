    ...
    return (
      <MainLayout>
        <div className="container mx-auto px-4 py-16 text-center">
          ...
          <p className="mb-8">The clinic you're looking for doesn't exist or has been removed.</p>
          <Button asChild>
            <a href="/services/clinics">Back to Clinics</a>
          </Button>
        </div>
      </MainLayout>
    );
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="lg:w-3/5">
              <div className="flex items-center mb-2">
                <div className={`px-3 py-1 rounded-full text-xs font-medium ${clinic.isOpen ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
              ...
              <h1 className="text-3xl font-bold mb-3">{clinic.name}</h1>
              ...
              <div className="flex flex-col gap-2 mb-6">
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-gray-700">{clinic.city}, {clinic.state} {clinic.zipCode}</p>
                    <p className="text-sm text-gray-500 mt-1">{clinic.distance} away</p>
                  </div>
                </div>
              ...
              {clinic.availability && (
                <div className="mb-6">
                  ...
                  <div className={`inline-block px-4 py-2 rounded-lg text-sm ${
                    clinic.availability.status === 'high' ? 'bg-green-100 text-green-800' :
                    clinic.availability.status === 'medium' ? 'bg-amber-100 text-amber-800' :
                    'bg-red-100 text-red-800'
                  }`}>
                    {clinic.availability.message}
                  </div>
                </div>
              )}
              ...
              <Card>
                <CardContent className="p-6">
                  ...
                  <div className="aspect-square rounded-lg bg-gray-200 mb-4 overflow-hidden">
                    <img 
                      src={`https://api.mapbox.com/styles/v1/mapbox/streets-v11/static/pin-s+627eea(${clinic.coordinates.lng},${clinic.coordinates.lat})/${clinic.coordinates.lng},${clinic.coordinates.lat},13,0/400x400?access_token=pk.eyJ1IjoiZXhhbXBsZSIsImEiOiJjbGVhcmVkLWNsZWFyZWQtY2xlYXIiLCJzZmEiOjB9.cleared-cleared-cleared`}
                      alt="Map location"
                      className="w-full h-full object-cover"
                    />
                  </div>
          ...
          <div>
            ...
            <div className="bg-gray-50 rounded-lg p-6 mb-8">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="text-2xl font-bold">{clinic.rating}</h3>
                  <div className="flex items-center mt-1">
                    {Array(5).fill(0).map((_, i) => (
                      <Star 
                        key={i} 
                        className={`w-4 h-4 ${i < Math.floor(clinic.rating) ? 'text-yellow-400 fill-yellow-400' : 'text-gray-300'}`} 
                      />
                    ))}
                    <span className="ml-2 text-sm text-gray-500">{clinic.reviewCount} reviews</span>
                  </div>
                </div>
              ...
              {[1, 2, 3].map((i) => (
                <div key={i} className="border-b pb-6">
                  <div className="flex justify-between mb-2">
                    <h3 className="font-medium">Jane S.</h3>
                  ...
                  <p className="text-gray-700">
                    The doctors are very professional and took time to listen to my concerns.
                    The facilities are clean and modern, and I didn't have to wait long.
                  </p>
                </div>
              ))}
            </div>
